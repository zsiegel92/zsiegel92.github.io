...
- Recent Hobby/Open-Source Software Projects
  - (2026) [`jevlint`](https://github.com/zsiegel92/jevlint) - Minimal implementation of a performant linter that uses [TypeSafe AI's September 2026 "_Jev_" model release](https://typesafe.ai) to zero-shot classify violations of markdown-specified lint rules for every file of a codebase. Checks rules in a single model pass over shared state (the file contents); caches per-rule, per-file output in on-disk serialized store (redb); does a second-pass over failed linted files to identify line number ranges; supports static analysis gate for AI linting (e.g. type-checking and other linting commands that must pass on a per-file basis before AI linting is attempted). Core CLI and file-watching server implemented in Rust. VS Code extension included.
  ...
  - (2026) [Hot Desk Booker](https://hot-desk-booker.vercel.app/) ([GitHub](https://github.com/zsiegel92/desk-booker)) - Plan ahead with team for onsite/WFH days to ensure a hot desk pod has capacity.
  - (2026) [`eightctl-modal`](https://github.com/zsiegel92/eightctl-modal) - E-ink-optimized web UI for Eight Sleep mattress thermostat.
  - (2026) [`aa-nt-app`](https://aa-nt.vercel.app) ([GitHub](https://github.com/zsiegel92/aa-nt)) - Opinionated scientific computation tool to convert amino acid sequences each into multiple nucleotide sequence variants for a genomic design setting.
